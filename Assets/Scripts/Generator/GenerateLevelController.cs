...
{
    private const int CountWall = 4;//будет использована при сглаживании пустот в стенах
    ...
    {
        _tileMapGround = view.TileMapGround;//так делаем, потому что что будем часто обращаться
    ...
    {
        //заполняем ячейки двумерного массива
        RandomFillLevel();
    ...
    private void RandomFillLevel()
    {
        ...
        for (var x = 0; x < _wightMap; x++)
        {
            ...
            {
                if (x == 0 || x == _wightMap - 1 || y == 0 || y == _heightMap - 1)//границы: стены, пол и потолок -  обязательно заполнены спрайтом
                    _map[x, y] = 1;
                else
